...
class Mercado:
    def __init__(self, preco_inicial, nome):
        ...
        # --- SINCRONISMO: Cada empresa tem seu próprio cadeado ---
        self.lock = threading.Lock()

    def comprar(self, trader_nome, quantidade):
        # O bloco 'with self.lock' define a SEÇÃO CRÍTICA
        with self.lock:
            ...
            # O registro da ordem também deve acontecer dentro ou logo após o lock
            registrar_ordem(
                trader_nome,
                self.nome,
                "COMPRA",
    ...
    def vender(self, trader_nome, quantidade):
        with self.lock:  # Protege contra falência simultânea e erro de preço
            if not self.ativo_valido:
                return
            ...
            if self.preco_atual <= 0:
                ...
                # ... lógica de venda normal

            if self.preco_atual < self.menor_valor:
                self.menor_valor = self.preco_atual

            # Atualiza o histórico e registros
            self.historico_precos.append(self.preco_atual)
            self.historico_precos = self.historico_precos[-18:]

            registrar_ordem(trader_nome, self.nome, self.ultima_acao,
                            quantidade, self.preco_atual, "red")
            registrar_trader(trader_nome, "VENDA", quantidade)


# --- LOCKS GLOBAIS ---
# Criamos um lock para proteger o dicionário de estatísticas dos traders
LOCK_PAINEL = threading.Lock()
# Criamos um lock para proteger a lista que exibe as ordens no terminal
LOCK_FITA = threading.Lock()


def registrar_trader(trader_nome, acao, quantidade):
    # SEÇÃO CRÍTICA: Acesso ao dicionário global PAINEL_TRADERS
    with LOCK_PAINEL:
...
def registrar_ordem(trader_nome, mercado_nome, acao, quantidade, preco, estilo):
    ...
    # SEÇÃO CRÍTICA: Alteração da lista global FITA_ORDENS
    with LOCK_FITA:
        FITA_ORDENS.append((mensagem, estilo))
        # Mantém apenas as últimas 12 mensagens para não sobrecarregar a memória
        del FITA_ORDENS[:-12]
...
def painel_mercado(mercados):
    ...
    for mercado in mercados:
        ...
        valorizacao = (
            (mercado.preco_atual - mercado.preco_inicial) / mercado.preco_inicial) * 100
...
def painel_resumo(mercados, nomes_traders, rodadas, inicio):
    ...
    total_operacoes = sum(dados["operacoes"]
                          for dados in PAINEL_TRADERS.values())
    volume_total = sum(dados["volume"] for dados in PAINEL_TRADERS.values())
    alvo_operacoes = len(nomes_traders) * rodadas
    tempo_decorrido = time.time() - inicio
    empresas_vivas = [
        mercado.preco_atual for mercado in mercados if mercado.ativo_valido]
    ...
    if empresas_vivas:
        grade.add_row(
            "Media ativa", f"R${sum(empresas_vivas) / len(empresas_vivas):.2f}")
    else:
...
def montar_dashboard(mercados, nomes_traders, rodadas, inicio):
    ...
    layout["resumo"].update(painel_resumo(
...
if __name__ == "__main__":
    ...
    for i in range(quantidade_empresas):
        EMPRESAS_INICIAIS.append((f"Empresa_{i+1}", 150.0))

    mercados = [Mercado(preco_inicial, nome)
    ...
    nomes_traders = []
    for i in range(quantidade_traders):
        nomes_traders.append(f"Trader_{i+1}")
    ...
    if RICH_DISPONIVEL:
        with Live(
            montar_dashboard(mercados, nomes_traders, rodadas, inicio),
